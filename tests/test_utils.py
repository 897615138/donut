...
from donut.utils import mini_batch_slices_iterator, BatchSlidingWindow
...
class MiniBatchSlicesIteratorTestCase(unittest.TestCase):
    ...
    def test_minibatch_slices_iterator(self):
        ...
        self.assertEqual(
            list(mini_batch_slices_iterator(9, 10, False)),
            [slice(0, 9, 1)]
        )
        self.assertEqual(
            list(mini_batch_slices_iterator(10, 10, False)),
            [slice(0, 10, 1)]
        )
        self.assertEqual(
            list(mini_batch_slices_iterator(10, 9, False)),
            [slice(0, 9, 1), slice(9, 10, 1)]
        )
        self.assertEqual(
            list(mini_batch_slices_iterator(10, 9, True)),
            [slice(0, 9, 1)]
        )

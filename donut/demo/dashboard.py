...
if button_pd:
    ...
    sl.text("训练数据量：{}，有{}个标注，测试数据量：{}，有{}个标注  \n【填充缺失数据,共用时{}】"
    ...
    start_time = time.time()
    test_score, epoch_list, lr_list, epoch_time = train_prediction(train_values, train_labels, train_missing,
                                                                   test_values,
    ...
    if catch_num != 0:
        ...
        if accuracy < 1:
            ...
            sl.text("未标记但超过默认阈值的点（数量：{}）：".format(len(special_anomaly_t)))
